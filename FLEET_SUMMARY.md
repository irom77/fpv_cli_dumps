...
Source data: `fpv_quads.csv` (full history) and `fpv_quads_latest.csv` (newest dump per quad, 27 quads). Values come from `diff all` dumps, which only record settings that differ from firmware defaults — a blank cell means the setting is at its firmware default, not that it is missing.
...
- **Class:** 14× whoop, 2× cinewhoop, 3× micro, 8× 5-inch. Size class inferred from craft name / board where `hardware.csv` doesn't set it.
- **Status:** 23× active, 2× broken, 2× retired. Lifecycle from `hardware.csv`; a blank there counts as active.
- **Discipline:** 5× race, 2× freestyle, 2× cinematic, 2× long-range, 16 unset. Hand-entered in `hardware.csv` (no heuristic — the dump gives no signal).
- **Flight controllers:** 14× F411, 6× F7X2, 2× G47X, 2× F405, 1× F745, 1× H743, 1× F411SX1280.
- **Firmware:** 12 on BF 4.4.x, 9 on BF 4.5.x, 4 on BF 4.2.x, 2 on BF 4.3.x.
- **ESC protocol:** 17× DSHOT300, 3× DSHOT600.
