...
_Auto-generated by the `fpv-fleet-update` skill on 2026-09-01 from the Betaflight CLI dumps in this folder._

Source data: `fpv_quads.csv` (full history) and `fpv_quads_latest.csv` (newest dump per quad, 32 quads). Values come from `diff all` dumps, which only record settings that differ from firmware defaults — a blank cell means the setting is at its firmware default, not that it is missing.
...
| openracer | 5-inch | race | active | HOBBYWING_XROTORF7CONV | F7X2 | 4.5.3.KAACK_V19 | - | - | - | 2026-08-12 |
| openracer2 | 5-inch | race | active | FOXEERF722V4 | F7X2 | 2025.12.3-alpha.KAACK_V19 | - | Analog | - | 2026-08-11 |
| prospec | 7-inch | race | active | HOBBYWING_XROTORF7CONV | F7X2 | 4.5.3.KAACK_V19 | - | - | - | 2026-09-01 |
...
- **Class:** 14× whoop, 2× cinewhoop, 6× micro, 9× 5-inch, 1× 7-inch. Size class inferred from craft name / board where `hardware.csv` doesn't set it.
- **Status:** 22× active, 4× broken, 3× retired, 3× incomplete. Lifecycle from `hardware.csv`; a blank there counts as active.
- **Discipline:** 10× race, 3× freestyle, 2× cinematic, 2× long-range, 15 unset. Hand-entered in `hardware.csv` (no heuristic — the dump gives no signal).
- **Flight controllers:** 14× F411, 8× F7X2, 4× F405, 2× G47X, 1× F745, 1× H743, 1× F411SX1280, 1× SPEEDYBEEF405MINI.
- **Firmware:** 12 on BF 4.4.x, 11 on BF 4.5.x, 5 on BF 4.3.x, 3 on BF 4.2.x, 1 on BF 2025.12.x.
...
_Showing the 12 active quads that have a `discipline` set; 20 others are hidden (retired, broken, incomplete, or no discipline recorded). "Needs attention" above still checks every quad._
...
| Quad | Center °/s | Max °/s | Expo | @50% | Preset | Type | Source |
|---|---|---|---|---|---|---|---|
| prospec | 70/70/70 | 670/670/670 | 0/0/0 | 185/185/185 | — | ACTUAL (default) | default |
...
| PROSPEC | 6S |  | HobbyWing XRotor Convertible F7 + 65A HD AM32 30x30 stack | HeadsUp \| Five33 2207 (MR30 connectors) |  | Runcam HDZero Nano 90 V2 | HDZero Race V3 HD | New 7-inch race quad; CLI craft name=prospec; current FC target HOBBYWING_XROTORF7CONV; Betaflight 4.5.3.KAACK_V19 as of 2026-09-01 (initial 2026-08-25 dump used HOBBYWING_XROTORF7CON on 4.3.2). Components from orders flyfive33 20003 and Pyrodrone 500322 (2026-07-31 / 2026-08-08). |
