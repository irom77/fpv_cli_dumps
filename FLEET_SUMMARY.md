...
_Auto-generated by the `fpv-fleet-update` skill on 2026-09-19 from the Betaflight CLI dumps in this folder._

Source data: `fpv_quads.csv` (full history) and `fpv_quads_latest.csv` (newest dump per quad, 34 quads). Values come from `diff all` dumps, which only record settings that differ from firmware defaults — a blank cell means the setting is at its firmware default, not that it is missing.
...
- **Status:** 23× active, 5× broken, 3× retired, 3× incomplete. Lifecycle from `hardware.csv`; a blank there counts as active.
...
- **ESC protocol:** 22× DSHOT300, 3× DSHOT600.
...
- **Video:** 12× HD, 2× Auto, 2× Analog. Quads showing `-` have `vcd_video_system` at firmware default in the diff — not necessarily video-less.
...
_Showing the 13 active quads that have a `discipline` set; 21 others are hidden (retired, broken, incomplete, or no discipline recorded). "Needs attention" above still checks every quad._
