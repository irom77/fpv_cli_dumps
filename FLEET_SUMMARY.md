...
_Auto-generated by the `fpv-fleet-update` skill on 2026-07-23 from the Betaflight CLI dumps in this folder._

Source data: `fpv_quads.csv` (full history) and `fpv_quads_latest.csv` (newest dump per quad, 30 quads). Values come from `diff all` dumps, which only record settings that differ from firmware defaults — a blank cell means the setting is at its firmware default, not that it is missing.
